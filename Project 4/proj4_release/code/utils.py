...
def get_image_paths(data_path, categories, num_train_per_cat=100, fmt='jpg', cross_validation = False):
  """
  ...
  if cross_validation is False:
    ...
    for cat in categories:
      ...
      pth_all.extend(pth)
      shuffle(pth_all)
      pth_train = pth_all[:num_train_per_cat]
      pth_test = pth_all[num_train_per_cat:2*num_train_per_cat]
      train_image_paths.extend(pth_train)
      train_labels.extend([cat]*len(pth_train))
      test_image_paths.extend(pth_test)
      test_labels.extend([cat]*len(pth_test))
...
def find_accuracy(train_image_paths, test_image_paths, train_labels, test_labels,
    categories, abbr_categories, predicted_categories):
  """
  shows the results
  :param train_image_paths:
