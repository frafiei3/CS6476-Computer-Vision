...
def get_tiny_images(image_paths):
  ...
  feats = np.zeros((len(image_paths), 256), float)

  #############################################################################
  # TODO: YOUR CODE HERE                                                      #
  #############################################################################

  for i in range(0, np.shape(image_paths)[0]):
    image = cv2.resize(load_image_gray(image_paths[i]), (16,16))
    image = np.reshape(image, (1,256))
    feats[i, :] = (image - np.mean(image))/np.std(image)
...
# np.shape(image_paths)[0]
  #############################################################################
  # TODO: YOUR CODE HERE                                                      #
  #############################################################################
  sampleSize = 40
  SIFTDescrpt = np.zeros((sampleSize * np.shape(image_paths)[0], dim), float)
  for i in range(0, np.shape(image_paths)[0]):
    frames, descriptors = vlfeat.sift.dsift(load_image_gray(image_paths[i]), fast = True, step = 20, float_descriptors = True)
    sample_idx = np.random.choice(np.shape(descriptors)[0], size = sampleSize)
    SIFT_sample = descriptors[sample_idx, :]
    SIFTDescrpt[sampleSize * i : sampleSize * (i + 1), :] = SIFT_sample

  vocab = vlfeat.kmeans.kmeans(SIFTDescrpt, vocab_size)
...
def get_bags_of_sifts(image_paths, vocab_filename):
  ...
  for i in range(np.shape(image_paths)[0]):
    frames, descriptors = vlfeat.sift.dsift(load_image_gray(image_paths[i]), fast = True, step = 10, float_descriptors = True)
    assignments = vlfeat.kmeans.kmeans_quantize(np.float32(descriptors), np.float32(vocab))
    hist, bin_edges = np.histogram(assignments, range = (1, np.shape(vocab)[0]), bins = np.shape(vocab)[0])
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats,
  ...
  # TODO: YOUR CODE HERE                                                      #
  #############################################################################
  k = 6 # k-nearest neighbor6
  D = sklearn_pairwise.pairwise_distances(test_image_feats, train_image_feats)
  NN_idx = np.argsort(D)[:, 0:k]
  for i in range(np.shape(NN_idx)[0]):
    labels = [train_labels[NN_idx[i, j]] for j in range(k)]
    vote = Counter(labels).most_common(2)
    if vote[0][1] < k:
      if vote[0][1] == vote[1][1]:
        test_labels.append(train_labels[NN_idx[i, 0]])
      else:
        test_labels.append(vote[0][0])
    else:
      test_labels.append(vote[0][0])
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
  ...
  # TODO: YOUR CODE HERE                                                      #
  #############################################################################
  train_labels_num = -np.ones((len(train_labels), len(categories)), int)
  for i in range(len(categories)):
    for j in range(len(train_labels)):
      if train_labels[j] == categories[i]:
        train_labels_num[j, i] = 1

  w = np.zeros((np.shape(train_image_feats)[1], len(categories)), float)
  b = np.zeros((1, len(categories)), float)
  for cat in range(len(categories)):
    model = LinearSVC(random_state = 0, tol = 1e-10, loss = 'hinge', C = 7, max_iter = 10000)
    model.fit(train_image_feats, train_labels_num[:, cat])
    w[:, cat] = model.coef_
    b[:, cat] = model.intercept_

  for att in range(np.shape(test_image_feats)[0]):
    confidence = np.dot(w.T, test_image_feats[att, :]) + b
    max_idx = np.argmax(confidence)
    test_labels.append(categories[max_idx])
