...
class SimpleNet(nn.Module):
  ...
  def __init__(self, num_classes, droprate=0.5, rgb=False, verbose=False):
    ...
    self.features = nn.Sequential(
      ...
      nn.MaxPool2d(kernel_size=5, stride=3, padding=0),
      nn.ReLU(),
...
def create_part2_model(model, num_classes):
  ...
  newLinearLayer = nn.Linear(in_features=4096, out_features=num_classes, bias=True)
  newLinearLayer.weight.data.normal_(0, 1)
  newLinearLayer.weight.data.mul_(1e-2)
  
  if newLinearLayer.bias is not None:
  	# Initializing biases with zeros.
  	newLinearLayer.bias.data.zero_()

  new_classifier.append(newLinearLayer)
  #######################################################################
  #                          END OF YOUR CODE                           #
  #######################################################################
